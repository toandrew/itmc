...
import com.infthink.itmc.type.BaseLocalFavorite;
import com.infthink.itmc.type.LocalMyFavoriteItemInfo;
import com.infthink.itmc.widget.MediaThumbView;
...
public class MyFavouriteAdapter extends BaseGroupAdapter<LocalMyFavoriteItemInfo> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        BaseLocalFavorite item = (BaseLocalFavorite)super.getItem(position);
        holder.videoTitle.setText(((LocalMyFavoriteItemInfo)item).mediaInfo.mediaName);
        holder.videoPlayDate.setText(getFavItemAddDate(item));
        return convertView;
    }

    private String getFavItemAddDate(
            BaseLocalFavorite fav) {
    
        if (fav == null)
            return "";
        long time = 0;
        if(fav.addDate != null){
            time = Long.parseLong(fav.addDate);
        } else{
            time = Long.parseLong(String.valueOf(Calendar.getInstance().getTimeInMillis()));
        }
